...
void rsa_new_key(EVP_PKEY *pkey, int bits) {
    ...
    if (!ctx) {
        fprintf(stderr, "EVP_PKEY_CTX_new_id(EVP_PKEY_RSA, NULL): ctx is not OK\n");
        exit(EXIT_FAILURE);
    }
    if (EVP_PKEY_keygen_init(ctx) <= 0) {
        fprintf(stderr, "EVP_PKEY_keygen_init(ctx) <= 0\n");
        exit(EXIT_FAILURE);
    }
    if (EVP_PKEY_CTX_set_rsa_keygen_bits(ctx, bits) <= 0) {
        fprintf(stderr, "EVP_PKEY_CTX_set_rsa_keygen_bits(ctx, bits) <= 0\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (EVP_PKEY_keygen(ctx, &pkey) <= 0) {
        /* Error */
        fprintf(stderr, "EVP_PKEY_keygen(ctx, &pkey) <= 0\n");
        exit(EXIT_FAILURE);
    }
}
...
int rsa_encrypt(EVP_PKEY *pkey,
    ...
    if (1 != EVP_SealInit(ctx, EVP_aes_256_cbc(), &encrypted_key,
        ...
        fprintf(stderr, "1 != EVP_SealInit(ctx, EVP_aes_256_cbc(), &encrypted_key, &encrypted_key_len, iv, &pkey, 1)\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (1 != EVP_SealUpdate(ctx, cipher_bytes, &len, bytes, bytes_len)) {
        fprintf(stderr, "1 != EVP_SealUpdate(ctx, cipher_bytes, &len, bytes, bytes_len)\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (1 != EVP_SealFinal(ctx, cipher_bytes + len, &len)) {
        fprintf(stderr, "1 != EVP_SealFinal(ctx, cipher_bytes + len, &len)\n");
        exit(EXIT_FAILURE);
    }
    cipher_bytes_len += len;
