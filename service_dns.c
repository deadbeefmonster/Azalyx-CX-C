...
service_dns_parse_request(guchar *buffer, struct dns_request *request, glong request_size) {
    ...
    g_debug("flags                   = %.16x", flags);
    g_debug("request->header->opcode = %.16x -> %.16x (%i)", header_mask_opcode, request->header->opcode,
    ...
    g_debug("flags                   = %.16x", flags);
    g_debug("request->header->rcode  = %.16x -> %.16x (%i)", header_mask_rcode, request->header->rcode,
...
service_dns_process_request(struct dns_request *request) {
    ...
    for (guint i = (request->header->qdcount - 1); i > -1; i--) {
        ...
        if (question->qtype == DNS_QTYPE_A) {
            ...
            g_debug("AXFR Request for '%s', need to look this up how it works... It should be TCP only",
                    question->qname->str);
